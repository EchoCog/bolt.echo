...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!provider || (provider !== 'openai' && provider !== 'anthropic')) {
      return json({ ok: false, error: "Invalid provider. Must be 'openai' or 'anthropic'" }, { status: 400 });
    }
     ...
     */
    let apiKey = provider === 'openai' ? process.env.OPENAI_API_KEY : process.env.ANTHROPIC_API_KEY;
    ...
    if (!apiKey) {
      return json({ ok: false, error: `API key for ${provider} is not configured` }, { status: 401 });
    }
    ...
    const content = await generateWithProvider(provider, apiKey, {
      model,
      messages,
    });
